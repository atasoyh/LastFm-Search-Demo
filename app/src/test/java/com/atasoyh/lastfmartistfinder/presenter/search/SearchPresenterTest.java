...
public class SearchPresenterTest {
    ...
    public void setUp() throws Exception {
        ...
        when(api.search(eq("lessData"),anyInt(),anyInt())).thenReturn(new io.reactivex.Observable<Response>() {
            @Override
            protected void subscribeActual(Observer<? super Response> observer) {
                Gson gson=new Gson();
                Response response=gson.fromJson(mockDataLess,Response.class);
                observer.onNext(response);
                observer.onComplete();
            }
        });

        when(api.search(eq("moreData"),anyInt(),anyInt())).thenReturn(new io.reactivex.Observable<Response>() {
            @Override
            protected void subscribeActual(Observer<? super Response> observer) {
                Gson gson=new Gson();
                Response response=gson.fromJson(mockDataMore,Response.class);
                observer.onNext(response);
                observer.onComplete();
            }
        });
